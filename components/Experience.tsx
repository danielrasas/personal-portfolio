          ...
          {EXPERIENCES.map((exp, index) => (
            <div key={exp.id} className="relative pl-8 md:pl-0">
              ...
              <div className={`md:flex items-center justify-between mb-2 ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                ...
                <div className={`md:w-5/12 bg-white p-5 rounded-2xl shadow-sm border border-slate-100 transition-all hover:shadow-md ${index % 2 === 0 ? 'md:text-left' : 'md:text-right'}`}>
                  ...
                  <p className="text-indigo-600 font-medium mb-2 text-sm">{exp.company}</p>
                  <ul className={`text-slate-600 text-sm space-y-1 list-disc list-inside ${index % 2 === 0 ? 'text-left' : 'md:text-right md:list-none'}`}>
